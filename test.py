...
class GetGreibach(object):
            ...
            for p in self.P_list:
                ...
                for i, word in enumerate(p[1:]):
                    if word is '@' or word is '':
                        ...
                        p[i+1] = '$'
                        break
                    if word not in self.N_:
                        is_N_ = False
                    if i == len(p) - 2 and is_N_ is True:
                        self.N_.append(p[0])
                        flag = True
                        break
            # print(self.N_)
        # self.unique_1d_list(self.N_)
        # print('N_:', self.N_)
        # print("遍历后：", self.N_)
    ...
    def search_n(self):
        temp_List = []
        # print("P:", self.P_list)
        for iii, p in enumerate(self.P_list):
            new_p = [p[0]]
            for ii, word in enumerate(p[1:]):
                new_p.append(word)
                for letter in list(word):
                    if letter in self.N_:
                        new_p.append(self.replace_word(word, letter))
            temp_List.append(new_p)

        flag = True
        while flag:
            flag = False
            for iii, p in enumerate(temp_List):
                # new_p = [p[0]]
                for ii, word in enumerate(p[1:]):
                    # new_p.append(word)
                    for letter in list(word):
                        if letter in self.N_ and self.replace_word(word, letter) not in p:
                            p.append(self.replace_word(word, letter))
                            flag = True
            # print("NEW P:", temp_List)
        # self.unique_2d_list(temp_List)
        self.New_P_list = temp_List
        # print("NEW P:", self.New_P_list)
        # print(len(self.New_P_list))

    def recover(self):
        # print(p_copy)
        for i in range(0, len(self.New_P_list)):
            for j in range(0, len(self.New_P_list[i])):
                if self.New_P_list[i][j] == '$':
                    del self.New_P_list[i][j]
        # print(p_copy)
    ...
    对于以终结符开头的后继，将终结符替换为对应的后继，直至为终结符号开头；
